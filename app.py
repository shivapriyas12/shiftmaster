...
# This is the "Gatekeeper" login for the whole website
MASTER_USER = "admin"
MASTER_PASS = "fcpl_user@2025"

# HR Tab specific password
HR_PASS = "hr@123"
...
# --- 5. MAIN APPLICATION UI (Authorized Only) ---
...
with tabs[1]:
    if not st.session_state.employees:
        ...
        if st.button("🚀 GENERATE MASTER SCHEDULE"):
            ...
            # Reset Schedules & Assign Weekly Off (Rotating)
            ...
            for sec_name, rules in st.session_state.line_rules.items():
                ...
                for idx, day in enumerate(dates):
                    is_sun = "Sun" in day
                    # Use Sunday total or Base + AB Cover logic
                    ...
                    for shift in ["A", "B", "C"]:
                        ...
                        for e in pool[:]:
                            ...
                            # Rest Rule: No A shift immediately after C shift
                            if shift == "A" and idx > 0 and e["Schedule"][dates[idx-1]] == "C": continue
                            if shift in e["Willing"]:
                                e["Schedule"][day] = shift
                                pool.remove(e)
                                count += 1
                    # Assign remaining staff to any available willing shift
                    for e in pool:
            ...
            st.download_button("📥 Download Excel Report", output.getvalue(), f"Shift_Report_{week_start}.xlsx")

# --- TAB 3: HR CONFIGURATION ---
with tabs[2]:
    st.subheader("Requirement Rules Management")
    hr_p = st.text_input("Enter HR Password to unlock", type="password")
    if hr_p == HR_PASS:
